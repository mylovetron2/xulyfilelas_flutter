...
class FileService {
  ...
  // Parse file LAS từ bytes và merge với dữ liệu TXT (cho web platform)
  static Future<Map<String, dynamic>> mergeTxtLasFromBytes(
    Uint8List lasBytes,
    String lasFileName,
    List<List<String>> dataRows,
  ) async {
    try {
      String content = utf8.decode(lasBytes);
      List<String> lines = content.split('\n');
      return _processLasLines(lines, dataRows);
    } catch (e) {
      return {
        'curveInfoList': [],
        'wellInfoList': [],
        'blockList': [],
        'success': false,
        'message': 'Lỗi khi merge TXT và LAS từ bytes: $e',
      };
    }
  }
  ...
  // Xử lý nội dung LAS và merge với dữ liệu TXT
  static Map<String, dynamic> _processLasLines(
    List<String> lines,
    List<List<String>> dataRows,
  ) {
    ...
    if (minTime == double.maxFinite.toInt() ||
      ...
      return {
        ...
        'blockList': [],
        'success': false,
        'message': 'Không tìm thấy TIME hợp lệ trong TXT!',
      };
    }
    ...
    if (blockList.isNotEmpty) {
      ...
      for (int i = 0; i < (blockList.length < 5 ? blockList.length : 5); i++) {
  ...
  // Tạo nội dung LAS file dạng string cho web platform
  static Future<String> generateLasContent(
